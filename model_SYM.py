...
def expiration(links,nodes,t0=0.001,DeltaT=0.1,DeltaP=100000,V_ini=CV,grad='naive',stop_crit=1e-4):
    ...
    while V >= V_min and dV > 0.000001:
        ...
        c+=1
        #dt_r_count =0
        iter = refresh_system(links,nodes,P,Q,t,it_lim=1000,DeltaP=DeltaP,stop_crit=stop_crit,V=V,DeltaT=dt,grad=grad)
        ...
        for i,id in enumerate(id_gen):
            ...
            Q_over_time[i].append(iter[2][id])
...
# for i in range(15):
#     Y = [D(x*1000,i)/DMAX(i) for x in X]
#     ax.plot(X,Y,label=f'D(P) for gen n°{i}')
#     plt.legend()
# ax.set_title('Relative diameter of vessels in function of transmural pressure\n for generation 0 to 15')
# ax.set_xlabel('Transmural Pressure, kPa')
# ax.set_ylabel('Relative diameter D/Dmax')
...
for COPD_ratio in [0,0.3,0.5,0.8,1]:
    ####SEVERE COPD INDIVIDUAL DATA####
    ...
    Y,X,Z,PoT,DoT,T,QoT = expiration(links,nodes,t0=t0,DeltaT=delta_t,V_ini=9e-3,DeltaP=Delta_P,grad='adapt',stop_crit=eps)
    ...
    ax.plot(LungVol,Debit,label=f'COPD severity : {COPD_ratio*100}%')
...
ax.invert_xaxis()
ax.set_title(f'Spirography Simulation for different COPD severities')
ax.set_xlabel('Volume (L)')
ax.set_ylabel('Debit (L/s)')
ax.set_xlim(10,1)
ax.set_ylim(-0.5,13)
