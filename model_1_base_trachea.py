...
def refresh_system(mat_link,mat_node,P_ini,Q_ini,t=0,DeltaP=100000,SAT=False,epsilon=[1e-2,1e-6],it_lim=1e3,V=CV,DeltaT=0.1):  
    ...
    sat = np.array([0.1 for k in range(len(Q[1:]))] + [10 for k in range(1,len(P)-N_f)])
    ...
    while norm_corr(F,N_Q+1)>=1e-4 or start == 1:
        ...
        P = [P_atm + P_pl]+list(X[len(Q[1:]):])+[P_alv(t,V,Phi)for k in range(N_f)] #CARE need to update VL_t
    ...
    return X,[P_atm]+P,Q,Q[0]*DeltaT

    
def expiration(links,nodes,T_series,DeltaP=100000,V_ini=CV):
    ...
    while V >= V_min and dV >=0.00001:
        t = T_series[c]
        c+=1
        iter = refresh_system(links,nodes,P,Q,t,it_lim=1000,DeltaP=DeltaP,DeltaT=DeltaT,V=V)
        DeltaT = T_series[c] - t
        if CHECK >= 2 : 
            print("DeltaT",DeltaT)
        dV = iter[-1]
        #print('piche',P)
        if CHECK>=1:
            print('------------------------------------------------------','\n iteration n°',c,'\n time:',t,'\n Initial Pressure :',P,'\n Pressure :',iter[1],'\n Initial Debit :',Q,'\n Debit :',iter[2],'\n Volume before step',V ,'\n expired volume during iteration:', iter[-1])
        V-=iter[-1] 
        print(' Volume after step :',V)
        Expired_Volume = V_ini-V
        Debits.append(iter[2][0])
        Volumes.append(Expired_Volume)
...
Rez = refresh_system(links,nodes,P,Q,t=Delta_t,DeltaP=10000,it_lim=1000)
print('New X---------------')
print(Rez[0])
print('New P---------------')
print(Rez[1])
...
# T = [0.05*k for k in range(1,10000)]
# T = [0.01*k for k in range(1,10)]+[0.1+0.02*k for k in range(100)]
# exp = expiration(links,nodes,T)
